...
const ColorItem: React.FC<ColorItemProps> = ({ name, code, image, id }) => (
    <div
        className={`w-[292px] h-[104px] p-[37px_24px_40px] mr-6 flex items-center justify-between ${image ? 'bg-center bg-cover bg-no-repeat' : ''}`}
        style={image ? { backgroundImage: `url(${image})` } : { backgroundColor: code }}
    >
        <div className="text-[20px] font-[Eina03] leading-normal tracking-normal text-white">{name}</div>
        <span className="text-[20px] font-normal font-[Eina03] leading-normal tracking-normal text-right text-white">{id}</span>
    </div>
);
        ...
        <>
            <div className='flex h-[689px]'>
                <div className='flex-col w-[316px]'>
                    <div className='w-[292px] h-[68px] bg-white shadow-md p-[12px_124px] hover:cursor-pointer'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44">
                            <g stroke="#000" strokeWidth="2.67">
                                <path d="M16 19L22.069 25 28 19" transform="translate(124 264)" />
                            </g>
                        </svg>
                    </div>
                    {colors.map((color) => (
